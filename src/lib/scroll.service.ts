...
export function scrollToProject(projectId: string, options: ScrollOptions = {}) {
  ...
  if (!element) {
    console.log(`[SCROLL_SERVICE] Élément non trouvé dans la Map pour ${projectId}, recherche dans le DOM...`);
    ...
    if (elements.length > 0) {
      ...
      console.log(`[SCROLL_SERVICE] Élément trouvé dans le DOM et enregistré pour ${projectId}`);
    } else {
      console.error(`[SCROLL_SERVICE] Aucun élément trouvé pour le projet ${projectId}`);
      return;
    }
  }
  ...
  console.log(`[SCROLL_SERVICE] Défilement vers le projet ${projectId} effectué`);
}
